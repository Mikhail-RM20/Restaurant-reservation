...
Так же принял решения о том, что нужно отказаться от наследования класса в Pydantic, так как они выполняют разные задачи(Хотя с другой стороны я допустил ошибку, ибо пришлось доублировать те же данные).
